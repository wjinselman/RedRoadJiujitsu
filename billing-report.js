...
export function renderReport({selected,previous,rows,previousRows,expected,today},print=false){
  ...
  const legacyNote=expected.legacyPaid?`<p class="billing-footnote">${expected.legacyPaid} active account${expected.legacyPaid===1?' has':'s have'} Paid status without a paid-on date. These accounts are not assigned to a payment month.</p>`:'';
  ...
  ${rows.length?(print?'<h3>Payment entries</h3>'+receiptTable:`<details class="billing-details"><summary>View ${rows.length} payment ${rows.length===1?'entry':'entries'}</summary>${receiptTable}</details>`):'<div class="billing-empty"><strong>Set a Paid on date in Edit</strong><p>Saving a payment date records the member in that calendar month. Paid / Unpaid status can still be changed separately. No online checkout is needed.</p></div>'}
